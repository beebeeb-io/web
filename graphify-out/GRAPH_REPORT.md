...
## Corpus Check
- 327 files · ~331,612 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1758 nodes · 2446 edges · 57 communities detected
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 539 edges (avg confidence: 0.8)
...
- `request()` --calls--> `recoverOpaqueRegister()`  [INFERRED]
  packages/shared/src/api/request.ts → src/lib/api.ts
- `request()` --calls--> `resendVerification()`  [INFERRED]
  packages/shared/src/api/request.ts → src/lib/api.ts
...
Cohesion: 0.03
Nodes (86): handleResend(), handleVerify(), handleNewFolder(), handleSubmit(), handleCopy(), handleDelete(), handleDownload(), handleRestore() (+78 more)

### Community 2 - "Community 2"
Cohesion: 0.04
Nodes (70): attachFreshToken(), listFiles(), listFilesPage(), opaqueLoginFinish(), opaqueLoginStart(), autoUpgradeToV1(), computeRecoveryCheck(), decryptChunk() (+62 more)
...
Cohesion: 0.05
Nodes (39): getApiUrl(), decryptAll(), loadNames(), downloadSharedFile(), downloadVersion(), getFileRequestPublic(), uploadToFileRequest(), canStreamToServiceWorker() (+31 more)
...
- **Thin community `Community 79`** (5 nodes): `Avatar()`, `formatRelativeDate()`, `getInitials()`, `SecuredBadge()`, `public-profile.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (4 nodes): `blockDevAutoLogin()`, `signUp()`, `export-resume.spec.ts`, `uniqueEmail()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (4 nodes): `blockDevAutoLogin()`, `signUp()`, `cli-auth-redirect.spec.ts`, `uniqueEmail()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 90`** (4 nodes): `reachPasswordStep()`, `sha1Upper()`, `onboarding-breach-check.spec.ts`, `uniqueEmail()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 107`** (3 nodes): `ApiError`, `.constructor()`, `encrypted-upload-v2-contract.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 119`** (3 nodes): `pwnedPasswordCount()`, `sha1HexUpper()`, `pwned-check.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 120`** (3 nodes): `deriveSasWords()`, `fnv1a()`, `sas-words.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `useToast()` connect `Community 4` to `Community 3`?**
