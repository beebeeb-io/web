...
## Corpus Check
- 298 files · ~308,915 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1671 nodes · 2271 edges · 51 communities detected
- Extraction: 79% EXTRACTED · 21% INFERRED · 0% AMBIGUOUS · INFERRED: 483 edges (avg confidence: 0.8)
...
3. `takeFromExternrefTable0()` - 28 edges
4. `withProxy()` - 28 edges
5. `passArray8ToWasm0()` - 24 edges
6. `encryptedUpload()` - 21 edges
7. `userFriendlyError()` - 21 edges
8. `SyncClient` - 19 edges
9. `passStringToWasm0()` - 18 edges
10. `getArrayU8FromWasm0()` - 16 edges
...
Cohesion: 0.02
Nodes (142): fireConnectionStatus(), fireErrorNotifier(), fireSessionExpired(), delay(), paceIfNeeded(), reportConnection(), request(), updateRateLimitState() (+134 more)
...
Cohesion: 0.05
Nodes (59): loadNames(), listFiles(), canStreamToServiceWorker(), createBlobSink(), createSwSink(), downloadAsZip(), expandToFiles(), computeRecoveryCheck() (+51 more)

### Community 3 - "Community 3"
Cohesion: 0.07
Nodes (60): addToExternrefTable0(), _assertBigInt(), _assertBoolean(), _assertNum(), compute_recovery_check(), debugString(), decodeText(), decompress_gzip() (+52 more)
...
Cohesion: 0.06
Nodes (34): getApiUrl(), downloadSharedFile(), downloadVersion(), uploadThumbnail(), uploadThumbnailLarge(), dispatchDecrypted(), awaitController(), canStreamToDisk() (+26 more)
...
Cohesion: 0.08
Nodes (30): handleCodeSubmit(), handlePasskeyUnlock(), handleRestore(), base64urlToBuffer(), bufferToBase64url(), credentialToAuthenticationJSON(), credentialToRegistrationJSON(), deletePasskey() (+22 more)
...
Cohesion: 0.15
Nodes (12): encryptedUpload(), withNetworkRetry(), computeFingerprint(), findByFingerprint(), getActiveUploads(), openDB(), removeUploadState(), saveUploadState() (+4 more)
...
- **Thin community `Community 41`** (8 nodes): `computeEta()`, `computeSpeed()`, `formatBytes()`, `formatEta()`, `formatSpeed()`, `pipelineStage()`, `stageLabel()`, `upload-progress.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 55`** (7 nodes): `decryptAll()`, `displayName()`, `handleBreadcrumbNav()`, `handleConfirm()`, `handleFolderOpen()`, `handleKey()`, `move-modal.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 98`** (4 nodes): `ContextMenu()`, `isPreviewable()`, `context-menu.tsx`, `preview.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 100`** (4 nodes): `actionMeta()`, `handleLoadMore()`, `relativeTime()`, `activity.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 106`** (3 nodes): `ApiError`, `.constructor()`, `encrypted-upload-v2-contract.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `formatBytes()` connect `Community 4` to `Community 1`, `Community 6`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Why does `handleFolderFilesSelected()` connect `Community 1` to `Community 11`, `Community 4`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Are the 111 inferred relationships involving `request()` (e.g. with `getApiUrl()` and `fireErrorNotifier()`) actually correct?**
  _`request()` has 111 INFERRED edges - model-reasoned connections that need verification._
