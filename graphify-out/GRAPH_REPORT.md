...
## Corpus Check
- 339 files · ~354,225 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1828 nodes · 2577 edges · 54 communities detected
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 559 edges (avg confidence: 0.8)
...
- `recoverOpaqueRegister()` --calls--> `request()`  [INFERRED]
  src/lib/api.ts → packages/shared/src/api/request.ts
- `unlockAccount()` --calls--> `request()`  [INFERRED]
  src/lib/api.ts → packages/shared/src/api/request.ts
- `resendVerification()` --calls--> `request()`  [INFERRED]
  src/lib/api.ts → packages/shared/src/api/request.ts
...
Cohesion: 0.02
Nodes (147): clearSessionConfirmed(), fireConnectionStatus(), fireErrorNotifier(), fireSessionExpired(), markSessionConfirmed(), wasSessionConfirmed(), delay(), paceIfNeeded() (+139 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (97): handleResend(), handleVerify(), handleNewFolder(), handleSubmit(), handleCopy(), copyToClipboard(), handleCopy(), handleRevoke() (+89 more)

### Community 2 - "Community 2"
Cohesion: 0.04
Nodes (82): attachFreshToken(), buildRequest(), opaqueLoginFinish(), opaqueLoginStart(), autoUpgradeToV1(), computeRecoveryCheck(), decryptChunk(), decryptFilename() (+74 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (62): getApiUrl(), downloadBundleItem(), downloadSharedFile(), downloadVersion(), getFileRequestPublic(), uploadThumbnail(), uploadThumbnailLarge(), uploadToFileRequest() (+54 more)

### Community 4 - "Community 4"
Cohesion: 0.07
Nodes (70): addHeapObject(), compute_recovery_check(), debugString(), decodeText(), decompress_gzip(), decrypt_chunk(), decrypt_chunks(), decrypt_metadata() (+62 more)
...
Cohesion: 0.08
Nodes (10): load(), onUpdated(), savePermissions(), setExpiry(), patchInvite(), commitQuery(), handleSubmit(), loadRecent() (+2 more)
...
- **Thin community `Community 41`** (8 nodes): `computeEta()`, `computeSpeed()`, `formatBytes()`, `formatEta()`, `formatSpeed()`, `pipelineStage()`, `stageLabel()`, `upload-progress.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 57`** (6 nodes): `blob()`, `createFolder()`, `deleteFile()`, `listChildIds()`, `listRootFolderIds()`, `folder-pagination.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 88`** (4 nodes): `blockDevAutoLogin()`, `signUp()`, `cli-auth-redirect.spec.ts`, `uniqueEmail()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (4 nodes): `escapeRegex()`, `signupAndUnlock()`, `refresh-stability.spec.ts`, `uniqueEmail()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 95`** (4 nodes): `ContextMenu()`, `isPreviewable()`, `context-menu.tsx`, `preview.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 108`** (3 nodes): `createBundleShareLink()`, `selectRow()`, `bundle-share.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 122`** (3 nodes): `ch()`, `migratePreferences()`, `notifications.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 129`** (2 nodes): `WasmChunkEncryptor`, `beebeeb_wasm.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `showToast()` connect `Community 1` to `Community 0`, `Community 2`, `Community 3`, `Community 6`, `Community 8`, `Community 9`, `Community 13`, `Community 15`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `handleFolderFilesSelected()` connect `Community 1` to `Community 2`, `Community 3`, `Community 5`?**
