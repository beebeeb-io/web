 ...
 * (no gate) skips this spec's tests rather than false-passing/false-failing;
 * see the gate probe below.
 *
...
test.describe('Pilot key gate — full registration flow (task 1411)', () => {
  ...
  test('wrong pilot key: registration is rejected with the typed message and the account is never created', async ({
    ...
    // sets pilotKeyError, which signup.tsx passes to BBInput's `error` prop
    // on the pilot-key-input field).
    //
    // MUST be scoped to that field's own error <p>, not any <p> on the page —
    // the permanent "Beebeeb is in private development... you'll need an
    // access key" notice is ALWAYS rendered on /signup (gate on or off,
    // error or no error) and its wording overlaps "pilot"/"private
    // development"/"access key", so an unscoped text filter passes even when
    // the typed error never rendered (review finding, task 1411 PR #35).
    // BBInput renders `error` as a <p> that is a sibling of the input's
    // wrapper div, i.e. the input's grandparent's child — see
    // packages/shared/src/design/bb-input.tsx.
    ...
    await expect(pilotError).toHaveText(/pilot access key is required to sign up/i)
