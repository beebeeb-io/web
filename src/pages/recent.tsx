...
/** Group files by date group, preserving order within each group. */
function groupByDate(files: DriveFile[]): { label: string; files: DriveFile[] }[] {
...
export function Recent() {
  ...
  // Files are already sorted by updated_at DESC from the server
  ...
  async function handleBulkDownload() {
    ...
    if (filesToDownload.length === 0) {
      showToast({
        ...
        description: 'Select at least one file to download.',
      })
      return
    }
  ...
  function renderFileRow(file: DriveFile) {
    ...
    <DriveLayout>
      ...
      <div className="px-5 py-2.5 border-b border-line flex items-center gap-3">
        <div className="flex items-center gap-1.5 text-sm">
          <span className="font-semibold text-ink">Recent</span>
        </div>
        <form
      ...
      >
        {loading ? (
          ...
          grouped.map((group) => (
            <div key={group.label}>
              {/* Date group header */}
              <div className="px-5 py-2 border-b border-line bg-paper-2 flex items-center gap-2">
                ...
                {group === grouped[0] && files.length > 0 && (
      ...
      <div className="px-5 py-2 border-t border-line bg-paper-2 flex items-center gap-3.5 text-[11px] text-ink-3">
        <span className="font-mono">{files.length} recent file{files.length !== 1 ? 's' : ''}</span>
        <span>·</span>
